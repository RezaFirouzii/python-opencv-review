...
def put_on_sunglasses(img):
    ...
    for (x, y, w, h) in face_rects:
        ...
        imgROI = img[y: y + h, x: x + w]    # extracting the face part
...
def put_on_mask(img):
    ...
    for (x, y, w, h) in face_rects:
        ...
        layer = cv.resize(layer, (w, h))
        
        gray_layer = cv.cvtColor(layer, cv.COLOR_BGR2GRAY)
        res, mask = cv.threshold(gray_layer, 227, 250, cv.THRESH_BINARY_INV)
        mask = cv.medianBlur(mask, 5)
        mask_inv = cv.bitwise_not(mask)

        new_face = cv.bitwise_and(layer, layer, mask=mask)
        gap_img = cv.bitwise_and(imgROI, imgROI, mask=mask_inv)
        img[y: y + h, x: x + w] = cv.add(new_face, gap_img)
